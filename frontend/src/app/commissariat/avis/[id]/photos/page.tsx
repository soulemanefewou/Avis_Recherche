...
export default function CommissariatManagePhotosPage() {
  const params = useParams();
  const router = useRouter();
  const { user, loading: isLoading } = useAuth();
  const avisId = params.id as string;

  const [avis, setAvis] = useState<AvisInfo | null>(null);
  ...
  const fetchAvis = useCallback(async () => {
    try {
      const res = await api.get(`/api/commissariat/avis/${avisId}`);
      setAvis(res.data.data);
    } catch {
      setError("Impossible de charger cet avis.");
    } finally {
      setLoading(false);
    }
  }, [avisId]);
  ...
  useEffect(() => {
    if (!isLoading && (!user || !user.roles.includes("ROLE_COMMISSARIAT"))) {
      router.push("/");
      return;
    }
    fetchAvis();
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (const file of toUpload) {
        ...
        await api.post(`/api/avis-recherches/${avisId}/photos`, formData, {
      ...
      setError(apiErr.response?.data?.message || "Erreur lors du téléversement.");
    } finally {
      setUploading(false);
      e.target.value = "";
    }
    ...
    return (
      <div className="max-w-[1600px] mx-auto px-6 py-8 space-y-6">
        ...
        <div className="rounded-xl border border-[#1f2937]/50 bg-[#0e1420]/40 p-6 space-y-4 animate-pulse">
          ...
          <div className="h-40 bg-[#1f2937] rounded-xl" />
        </div>
        <div className="rounded-xl border border-[#1f2937]/50 bg-[#0e1420]/40 p-6 space-y-4 animate-pulse">
          <div className="h-4 bg-[#1f2937] rounded w-24" />
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            {[...Array(3)].map((_, i) => <div key={i} className="aspect-square bg-[#1f2937] rounded-xl" />)}
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 py-8 space-y-6">
      <div className="flex items-center gap-3 animate-fade-in">
        <Link href={`/commissariat/avis/${avisId}`} className="w-8 h-8 rounded-lg border border-[#1f2937]/80 bg-[#0e1420]/60 flex items-center justify-center hover:border-[#ef4444]/30 hover:bg-[#ef4444]/5 transition-all">
          <ChevronLeft className="h-4 w-4 text-gray-400" />
        </Link>
        <div>
          <h1 className="text-xl font-bold text-white tracking-tight">Gérer les photos</h1>
          <p className="text-sm text-gray-500">Ajoutez, supprimez ou changez l&apos;image principale</p>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-[#1f2937]/80 bg-[#0e1420]/60 backdrop-blur-sm p-6 animate-fade-in">
        ...
        <label className={`flex flex-col items-center justify-center border-2 border-dashed rounded-xl p-10 text-center transition-all ${
          atLimit
          ...
          <div className="p-3 bg-[#ef4444]/10 rounded-full mb-3">
            <Upload className="h-6 w-6 text-[#ef4444]" />
          </div>
          <p className="font-semibold text-sm text-gray-300">
            {atLimit ? "Nombre maximum atteint" : uploading ? "Envoi en cours..." : "Cliquez pour choisir des fichiers"}
          </p>
          <p className="text-xs text-gray-600 mt-1">
            {atLimit ? "Supprimez une photo pour en ajouter" : "PNG, JPG, WEBP — max 5 Mo — multi-sélection possible"}
          </p>
          {!atLimit && (
      ...
      <div className="rounded-xl border border-[#1f2937]/80 bg-[#0e1420]/60 backdrop-blur-sm p-6 animate-fade-in" style={{ animationDelay: "80ms", animationFillMode: "both" }}>
        ...
        {avis.photos && avis.photos.length > 0 ? (
              ...
              <div key={photo.id} className={`relative rounded-xl border overflow-hidden bg-[#0b0f17]/60 group ${photo.estPrincipale ? "border-[#ef4444]/50 ring-1 ring-[#ef4444]/20" : "border-[#1f2937]/50"}`}>
                <div className="aspect-[3/4] relative overflow-hidden bg-[#0b0f17]">
                  ...
                  <div className="absolute inset-0 bg-[#0b0f17]/80 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-3">
                    {!photo.estPrincipale && (
                      <button onClick={() => handleSetPrimary(photo.id)} className="w-full max-w-[130px] bg-white/10 hover:bg-white/20 text-white text-[11px] font-bold py-1.5 px-2 rounded-lg transition-all shadow-sm backdrop-blur-sm">
                        Définir principale
                      </button>
                    )}
                    <button onClick={() => setDeleteTarget(photo.id)} className="w-full max-w-[130px] bg-[#ef4444]/80 hover:bg-[#ef4444] text-white text-[11px] font-bold py-1.5 px-2 rounded-lg transition-all shadow-sm flex items-center justify-center gap-1">
          ...
          <div className="text-center py-12 bg-[#0b0f17]/30 rounded-xl border border-[#1f2937]/50">
